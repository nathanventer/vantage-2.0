...
begin
  ...
  if v_auditor is not null and not exists (
    select 1 from public.rate_subscriptions where user_id = v_auditor
  ) then
    insert into public.rate_subscriptions (user_id, plan, status, current_period_end)
...
begin
  ...
  if to_regclass('public.notifications') is not null and v_buyer is not null then
    ...
    if not exists (select 1 from public.notifications where user_id = v_provider and title = 'Quote accepted') then
      insert into public.notifications (user_id, sender_id, type, kind, title, body, link, read_at, metadata) values
    ...
    if not exists (select 1 from public.notifications where user_id = v_admin and title = 'Registration pending review') then
      insert into public.notifications (user_id, sender_id, type, kind, title, body, link, read_at, metadata) values
    ...
    if not exists (select 1 from public.notifications where user_id = v_auditor and title = 'Pulse demo active') then
      insert into public.notifications (user_id, sender_id, type, kind, title, body, link, read_at, metadata) values
    ...
    if not exists (select 1 from public.notifications where user_id = v_finance and title = 'Invoice generated') then
      insert into public.notifications (user_id, sender_id, type, kind, title, body, link, read_at, metadata) values
    ...
    if not exists (select 1 from public.notifications where user_id = v_warehouse and title = 'Goods received') then
      insert into public.notifications (user_id, sender_id, type, kind, title, body, link, read_at, metadata) values
    ...
    if not exists (select 1 from public.notifications where user_id = v_transport and title = 'Trip dispatched') then
      insert into public.notifications (user_id, sender_id, type, kind, title, body, link, read_at, metadata) values
    ...
    if not exists (select 1 from public.notifications where user_id = v_customs and title = 'Clearance released') then
      insert into public.notifications (user_id, sender_id, type, kind, title, body, link, read_at, metadata) values
