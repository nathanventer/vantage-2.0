...
function DemoRolePicker({ role, onChange }: { role: Role; onChange: (r: Role) => void }) {
  return (
    <div className="px-1 pb-1">
      ...
      <div className="flex rounded-lg border bg-inset p-0.5" role="tablist" aria-label="Switch demo role">
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className={cn(
            "group flex max-w-[min(100vw-8rem,16rem)] items-center gap-2.5 rounded-xl border border-transparent px-2 py-1.5 text-left outline-none transition",
        ...
        >
          ...
          <div className="hidden min-w-0 flex-1 sm:block">
            ...
            <div className="mt-0.5 flex items-center gap-1.5">
              ...
              {user.companyName && (
                <span className="truncate text-[11px] text-muted-foreground">{user.companyName}</span>
              )}
            </div>
          </div>
        ...
        <div className="border-b px-4 py-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {user.companyName && (
                <p className="mt-2 text-xs leading-snug text-muted-foreground">{user.companyName}</p>
              )}
            </div>
          </div>
        </div>
