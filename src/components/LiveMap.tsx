...
function corridor(a: [number, number], b: [number, number], seed: string, n = 64): [number, number][] {
  ...
  const cut =
    trip.status === "Delivered"
      ...
      : trip.status === "Scheduled"
        ? 0
        : Math.max(1, Math.min(path.length - 2, Math.round((trip.progressPct / 100) * path.length)));
  return { path, cut };
}
...
const GLYPHS = {
  ...
  border: '<path d="M4 22V4c0-.6.4-1 1-1h14c.6 0 1 .4 1 1v18"/><path d="M2 22h20"/><path d="M12 3v19"/>',
...
export function LiveMap({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const c of MAJOR_CITIES) {
        ...
        L.marker(c.coords, {
          icon: L.divIcon({ className: "", html: `<div class="map-city">${c.name}</div>`, iconAnchor: [-5, 8] }),
          interactive: false,
        }).addTo(map);
      }
      for (const p of PORTS) {
        L.marker(p.coords, {
          icon: L.divIcon({ className: "", html: glyphIconHtml("anchor", "#38bdf8"), iconSize: [22, 22], iconAnchor: [11, 11] }),
        })
          .addTo(map)
      ...
      for (const b of BORDER_POSTS) {
        L.marker(b.coords, {
          icon: L.divIcon({ className: "", html: glyphIconHtml("border", "#fbbf24"), iconSize: [20, 20], iconAnchor: [10, 10] }),
        })
          .addTo(map)
      ...
      for (const w of WAREHOUSE_HUBS) {
        L.marker(w.coords, {
          icon: L.divIcon({ className: "", html: glyphIconHtml("warehouse", "#a6b2c8"), iconSize: [20, 20], iconAnchor: [10, 10] }),
        })
          .addTo(map)
      ...
      for (const t of trips) {
        ...
        if (t.status === "In Transit") {
          L.polyline(path.slice(0, cut + 1), { color: o.color, weight: 1.5, opacity: 0.35 }).addTo(map);
        }
        const pos = path[cut];
      ...
      if (remaining.length > 1 && selected.status !== "Delivered") {
        L.polyline(remaining, { color: "#8494ad", weight: 2, opacity: 0.7, dashArray: "4 8" }).addTo(map);
      }
      ...
      const chip = (text: string) =>
        L.divIcon({ className: "", html: `<div class="map-chip">${text}</div>`, iconAnchor: [-8, 12] });
      L.circleMarker(path[0], { radius: 5, color: ops.color, weight: 2, fillColor: "#0f1626", fillOpacity: 1 }).addTo(map);
      ...
      L.marker(path[path.length - 1], { icon: chip(selected.destination), interactive: false }).addTo(map);
  ...
  return (
    <div ref={wrapperRef} className="vantage-map relative overflow-hidden rounded-xl border bg-inset">
      <div
        ref={containerRef}
      ...
      <div className="pointer-events-none absolute left-3 top-3 z-[1100] flex max-w-[70%] flex-wrap items-center gap-2">
        ...
        <div
          ...
          style={{ background: "rgba(15,22,38,0.9)", color: ops.color, borderColor: `${ops.color}44` }}
        >
          {ops.status}
      ...
      <div className="pointer-events-none absolute bottom-8 left-3 z-[1100] flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border bg-surface/90 px-3 py-1.5 text-[11px] shadow-sm backdrop-blur">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ background: "#2f6bff" }} /> On-time
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ background: "#fb923c" }} /> Delayed
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ background: "#a78bfa" }} /> Customs
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ background: "#34d399" }} /> Delivered
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ background: "#f87171" }} /> Risk
        </span>
      </div>
    </div>
  );
